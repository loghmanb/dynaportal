...
class FieldLabelTestCase(BaseTestCase):
    ...
    def setUp(self) -> None:
        """Setup."""
        self.template = "{% load common %}" "{% field_label id string bold %}"

    def test_normal_label(self):
        """Test normal label."""
        rendered = self.render_template(
            self.template, {"id": 123, "string": "Test label", "bold": False}
        )
        self.assertEqual(
            rendered, '<label class="govuk-label" for="123">\n  Test label\n</label>'
        )

    def test_bold_label(self):
        """Test bold label."""
        rendered = self.render_template(
            self.template, {"id": 123, "string": "Test label", "bold": True}
        )
        self.assertEqual(
            rendered,
...
class HintTestCase(BaseTestCase):
    ...
    def test_hint(self) -> None:
        ...
        self.assertEqual(
            rendered, '<div class="govuk-hint">Sample hint comes here.</div>'
        )
...
class FieldErrorTestCase(BaseTestCase):
    ...
    def test_field_error_label(self) -> None:
        ...
        rendered = self.render_template(
            template, {"string": "Sample serror text comes here!"}
        )
        self.assertEqual(
            rendered,
